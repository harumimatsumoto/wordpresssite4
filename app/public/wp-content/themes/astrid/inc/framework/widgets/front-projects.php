...
class Sdframework_Projects extends WP_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'atframework_projects_widget', 'description' => __( 'Show off your latest projects.', 'simpledesign') );
        parent::__construct(false, $name = __('Simpledesign FP: Projects', 'simpledesign'), $widget_ops);
	...
	<p><?php _e('This widget displays all pages that have the Single Project page template assigned to them.', 'simpledesign'); ?></p>
	<p><em><?php _e('Tip: to rearrange the projects order, edit each project page and add a value in Page Attributes > Order', 'simpledesign'); ?></em></p>
	<p>
	<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title', 'simpledesign'); ?></label>
	<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" />
	</p>
	<p><label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of projects to show (-1 shows all of them):', 'simpledesign' ); ?></label>
	<input id="<?php echo $this->get_field_id( 'number' ); ?>" name="<?php echo $this->get_field_name( 'number' ); ?>" type="text" value="<?php echo $number; ?>" size="3" /></p>
	<p><label for="<?php echo $this->get_field_id( 'offset' ); ?>"><?php _e( 'Offset (number of projects needs to be different than -1 for this option to work):', 'simpledesign' ); ?></label>
    ...
    <p><label for="<?php echo $this->get_field_id('see_all'); ?>"><?php _e('The URL for your button [In case you want a button below your projects block]', 'simpledesign'); ?></label>
    ...
    <p><label for="<?php echo $this->get_field_id('see_all_text'); ?>"><?php _e('The text for the button [Defaults to <em>See all our projects</em> if left empty]', 'simpledesign'); ?></label>
			...
			<?php if ( $title ) echo $before_title . $title . $after_title; ?>
					...
					<?php while ( $projects->have_posts() ) : $projects->the_post(); ?>
						<div class="project">
							...
							<div class="project-thumb">
								<?php the_post_thumbnail('simpledesign-project-thumb'); ?>
				...
				<?php if ($see_all != '') : ?>
					<a href="<?php echo esc_url($see_all); ?>" class="button centered-button">
						...
						<?php else : ?>
							<?php echo __('See all our projects', 'simpledesign'); ?>
						<?php endif; ?>
					</a>
				<?php endif; ?>				
	<?php
