...
class Sdframework_Clients extends WP_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'atframework_clients_widget', 'description' => __( 'Show a list of your clients.', 'simpledesign') );
        parent::__construct(false, $name = __('Simpledesign FP: Clients', 'simpledesign'), $widget_ops);
	...
	<p><?php _e('This widget displays all pages that have the Client Service page template assigned to them.', 'simpledesign'); ?></p>
	<p><em><?php _e('Tip: to rearrange the clients order, edit each client page and add a value in Page Attributes > Order', 'simpledesign'); ?></em></p>
	<p>
	<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title', 'simpledesign'); ?></label>
	<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" />
	</p>
	<p><label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of clients to show (-1 shows all of them):', 'simpledesign' ); ?></label>
	<input id="<?php echo $this->get_field_id( 'number' ); ?>" name="<?php echo $this->get_field_name( 'number' ); ?>" type="text" value="<?php echo $number; ?>" size="3" /></p>
	<p><label for="<?php echo $this->get_field_id( 'offset' ); ?>"><?php _e( 'Offset (number of clients needs to be different than -1 for this option to work):', 'simpledesign' ); ?></label>
    ...
    <p><label for="<?php echo $this->get_field_id('see_all'); ?>"><?php _e('The URL for your button [In case you want a button below your clients block]', 'simpledesign'); ?></label>
    ...
    <p><label for="<?php echo $this->get_field_id('see_all_text'); ?>"><?php _e('The text for the button [Defaults to <em>See all our clients</em> if left empty]', 'simpledesign'); ?></label>
			...
			<?php if ( $title ) echo $before_title . $title . $after_title; ?>
					...
					<?php while ( $clients->have_posts() ) : $clients->the_post(); ?>
						<div class="client">
							...
							<div class="client-thumb">
								<?php the_post_thumbnail('simpledesign-client-thumb'); ?>
							</div>
							<?php endif; ?>
						</div>
					<?php endwhile; ?>
				...
				<?php if ($see_all != '') : ?>
					<a href="<?php echo esc_url($see_all); ?>" class="button centered-button">
						...
						<?php else : ?>
							<?php echo __('See all our clients', 'simpledesign'); ?>
						<?php endif; ?>
					</a>
				<?php endif; ?>				
	<?php
