...
class Sdframework_Employees extends WP_Widget {

	public function __construct() {
		$widget_ops = array('classname' => 'atframework_employees_widget', 'description' => __( 'Show your employees', 'simpledesign') );
        parent::__construct(false, $name = __('Simpledesign FP: Employees', 'simpledesign'), $widget_ops);
	...
	<p><?php _e('This widget displays all pages that have the Single Employee page template assigned to them.', 'simpledesign'); ?></p>
	<p><em><?php _e('Tip: to rearrange the employees order, edit each employee page and add a value in Page Attributes > Order', 'simpledesign'); ?></em></p>
	<p>
	<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title', 'simpledesign'); ?></label>
	<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" />
	</p>
	<p><label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of employees to show (-1 shows all of them):', 'simpledesign' ); ?></label>
	<input id="<?php echo $this->get_field_id( 'number' ); ?>" name="<?php echo $this->get_field_name( 'number' ); ?>" type="text" value="<?php echo $number; ?>" size="3" /></p>
	<p><label for="<?php echo $this->get_field_id( 'offset' ); ?>"><?php _e( 'Offset (number of employees needs to be different than -1 for this option to work):', 'simpledesign' ); ?></label>
    ...
    <p><label for="<?php echo $this->get_field_id('see_all'); ?>"><?php _e('The URL for your button [In case you want a button below your employees block]', 'simpledesign'); ?></label>
    ...
    <p><label for="<?php echo $this->get_field_id('see_all_text'); ?>"><?php _e('The text for the button [Defaults to <em>See all our employees</em> if left empty]', 'simpledesign'); ?></label>
			...
			<?php if ( $title ) echo $before_title . $title . $after_title; ?>
				...
				<div class="employees-area clearfix">
					<?php while ( $employees->have_posts() ) : $employees->the_post(); ?>
						<div class="employee simpledesign-3col">
				...
				<?php if ($see_all != '') : ?>
					<a href="<?php echo esc_url($see_all); ?>" class="button centered-button">
						...
						<?php else : ?>
							<?php echo __('See all our employees', 'simpledesign'); ?>
						<?php endif; ?>
					</a>
				<?php endif; ?>				
	<?php
